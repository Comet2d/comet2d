...
pub fn position(self: *const cmt.mouse.Mouse) cmt.math.Vec2(i32) {
    ...
    return .{
        @intCast(@divFloor(@as(i32, @intCast(mouse_state.x)), self.comet.graphics.window_ratio)),
        @intCast(@divFloor(@as(i32, @intCast(mouse_state.y)), self.comet.graphics.window_ratio)),
    };
}
